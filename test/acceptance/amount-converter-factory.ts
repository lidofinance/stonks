...
describe('AmountConverterFactory: acceptance', async function () {
  it('should have correct chainlink feed registry', async function () {
    ...
    expect(feedRegistrySetEvents[0].args[0]).to.hexEqual(contracts.CHAINLINK_PRICE_FEED_REGISTRY)

    expect(await amountConverterFactory.FEED_REGISTRY()).to.hexEqual(
