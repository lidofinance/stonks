...
import { ChainLinkUsdTokensConverter, Stonks, Order } from "../../typechain-types"
...
describe("Happy path", function () {
    ...
    describe("order creation", async function () {
        ...
        it("should fill up stonks contract", async () => {
            ...
            expect(isClose(await stethTreasury.balanceOf(subjectAddress), value)).to.be.true
        })
        ...
        it("settlement should pull off assets from order contract", async () => {
            ...
            expect(isClose(await stethRelayer.balanceOf(orderAddress), BigInt(0))).to.be.true
        })
    })
